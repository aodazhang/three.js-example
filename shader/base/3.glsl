...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float mixer = step(0.5, uv.x);// 块状色
  mixer = uv.x;// 渐变色
  fragColor = vec4(mix(color1, color2, mixer), 1.0);

  // 2.水平重复 + 混合色
  mixer = step(0.5, fract(uv.x * 3.0));// 块状色
  mixer = fract(uv.x * 3.0);// 渐变色
  fragColor = vec4(mix(color1, color2, mixer), 1.0);

  // 3.圆形 + 混合色
  float dist = length(uv - vec2(0.5));
  mixer = step(0.25, dist);// 块状色
  mixer = dist;// 渐变色
  fragColor = vec4(mix(color1, color2, mixer), 1.0);

  // 4.水平垂直重复 + 圆形 + 混合色
  dist = length(fract(uv * 5.0) - vec2(0.5));
  mixer = step(0.25, dist);// 块状色
  mixer = dist;// 渐变色
  fragColor = vec4(mix(color1, color2, mixer), 1.0);

  // 5.对角线重复 + 混合色
  mixer = step(1.0, uv.x + uv.y);// 块状色（uv.x+uv.y 在对角线达到 1.0）
  mixer = (uv.x + uv.y) / 2.0;// 渐变色（除以 2 让渐变更平滑）
  mixer = 1.0 - (uv.x + uv.y) / 2.0;// 反向渐变色
  fragColor = vec4(mix(color1, color2, mixer), 1.0);

  // 6.对角线中轴重复 + 混合色
  float mixer1 = uv.x + uv.y;// 黄 -> 青
  float mixer2 = 2.0 - (uv.x + uv.y);// 青 -> 黄
