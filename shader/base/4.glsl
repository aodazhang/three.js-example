...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
  ...
  */
  // vec3 mask1=vec3(step(.5,uv.x));
  // vec3 mask2=vec3(step(.5,uv.y));
  vec3 mask1=vec3(step(.5,fract(uv.x*3.)));
  vec3 mask2=vec3(step(.5,fract(uv.y*3.)));
  ...
  mask1=vec3(step(.5,fract(uv.x*3.)));
  mask2=vec3(step(.5,fract(uv.y*3.)));
  vec3 mixer=abs(mask1-mask2);
  fragColor=vec4(mix(color1,color2,mixer),1.);
  
  // 3.圆环
  float strength=distance(uv,vec2(.5))-.25;// 获取向量 uv 和 vec2(0.5) 之间的距离
  strength=abs(strength);// 取绝对中让靠近中间的距离变为正数
  strength=aastep(.01,strength);// 通过限制过渡，形成圆环（这里采用 aastep 代替 step，作用为抗锯齿）
  fragColor=vec4(vec3(strength),1.);
  
  // 4.对角线
  float mixer1=1.-abs((uv.x+uv.y-1.));// uv.x+uv.y-1.0 范围限制在 [-1, 1]，通过绝对值限制为 [1, 1]，在利用 1 - 该值得到对角线效果
  fragColor=vec4(vec3(mixer1),1.);
}
