...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 2.随时间变化半径 + 圆形
  fragColor = vec4(vec3(step(abs(sin(iTime) * 0.5), dist)), 1.0);
  ...
  dist = length(fract(uv * 5.0) - vec2(0.5));
  fragColor = vec4(vec3(step(abs(sin(iTime + uv.x + uv.y) * 0.5), dist)), 1.0);

  // 6.径向重复（先居中后重复）
  // dist = fract(length(uv - vec2(.5)) * 5.);// 默认 length(uv-vec2(.5)) 的最大值不是 (0, 0) ~ (1, 1) 而是 (0.5, 0.5) ~ (1, 1)，也就是 √2/2=0.707，因此可以先除以 0.707 再翻倍取小数
  dist = fract(length(uv - vec2(0.5)) / 0.707 * 5.0);
  fragColor = vec4(vec3(step(0.5, dist)), 1.0);

  // 7.径向重复（先居中后重复） + 随时间变化半径
  dist = fract((length(uv - vec2(0.5)) / 0.707 + iTime) * 5.0);
  fragColor = vec4(vec3(step(0.5, dist)), 1.0);
}
